...
--sehre gore isim sayisini bulunuz.
...
ORDER BY maas DESC --burada buyukten kucuge siraladik.
...
drop table if exists personel; --onceki personel tablosunu siliyoruz.
...
/*
							UNION ALL:
UNION islemi 2 veya daha cok SELECT isleminin sonuc KUMELERINI birlestirmek 
icin kullanilir, Ayni kayit birden fazla olursa, sadece bir tanesini alir. 
...
--SQL INTERSECT (kesisim) operatörü, 2 veya daha fazla SELECT ifadesinin sonuçlarını 
--döndürmek için kullanılır. Ancak, yalnızca tüm sorgular veya veri kümeleri 
--tarafından seçilen satırları döndürür. Bir sorguda bir kayıt varsa ve diğerinde 
--yoksa, INTERSECT sonuçlarından çıkarılacaktır.
...
select id from personel where maas not between 4000 and 5500
intersect
select id from personel_bilgi where cocuk_sayisi in(2,3);
